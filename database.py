...
with con:
	...
	cur.execute("SELECT name, state, year, warm_month, cold_month FROM cities INNER JOIN weather ON name")
	
	# Find hottest cities in July
	
	cur.execute("SELECT name, state, warm_month, AVG(average_high) FROM weather ORDER BY warm_month = July HAVING AVG(average_high) > 65")
